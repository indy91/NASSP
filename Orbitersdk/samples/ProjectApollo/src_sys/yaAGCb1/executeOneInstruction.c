...
executeOneInstruction(FILE *logFile)
{
      ...
      for (i = 0; i < numBreaksOrWatches; i++)
        {
          if (breaksOrWatches[i] == BREAK_UININITIALIZED
              ...
              && agc.memory[flatOperandAddress] == 0
   ...
   */
    {
      //int overflow = 0;
      if (agc.countMCT * 3 > agc.nextTimerIncrement)
        {
		  agc.nextTimerIncrement += 2560;
...
void
UnprogrammedIncrement(agcBlock1_t *State, int Counter, int IncType)
{
	int16_t *Ch;
	int Overflow = 0;
	Counter &= 0x7f;
	Ch = &State->memory[Counter];
	switch (IncType)
	{
	case 0:
