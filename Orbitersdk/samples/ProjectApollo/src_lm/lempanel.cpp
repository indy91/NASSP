...
void LEM::SetSwitches(int panel) {
	...
	LtgORideAnunSwitch.Init(323, 168, 34, 29, srf[SRF_SWITCHUP], srf[SRF_BORDER_34x29], Panel5SwitchRow, &lca.NumDockCompLTGFeeder, &lca.CW_DC_Dimmer_Circuit);
...

{
	...
	case AID_DC_BUS_LIGHT:
		if (lca.Fixed_55V_Output.Voltage() > 2.25 && (LampToneTestRotary.GetState() == 6)) {								//Needs control logic to indicate DC bus or main feeder grounded
			oapiBlt(surf, srf[SRF_RR_NOTRACK], 0, 0, 0, 34, 34, 34, SURF_PREDEF_CK); // Light On
		}
		else {
			oapiBlt(surf, srf[SRF_RR_NOTRACK], 0, 0, 0, 0, 34, 34, SURF_PREDEF_CK); // Light Off
	...
	case AID_BAT_FAULT_LIGHT:
		if (LtgORideAnunSwitch.Voltage() > 2.25 && (LampToneTestRotary.GetState() == 6 || scera2.GetBatFaultLogic())) {
			oapiBlt(surf, srf[SRF_RR_NOTRACK], 0, 0, 0, 34, 34, 34, SURF_PREDEF_CK); // Light On
		}
		else {
			oapiBlt(surf, srf[SRF_RR_NOTRACK], 0, 0, 0, 0, 34, 34, SURF_PREDEF_CK); // Light Off
	...
	case AID_SEQ_LIGHT1:
		if (lca.Fixed_55V_Output.Voltage() > 2.25 && (scera1.GetVoltage(12, 11) > 2.5 && stage < 2 || LampToneTestRotary.GetState() == 6)) {
	...
	case AID_SEQ_LIGHT2:
		if (lca.Fixed_55V_Output.Voltage() > 2.25 && (scera1.GetVoltage(12, 12) > 2.5 || LampToneTestRotary.GetState() == 6)) {
			oapiBlt(surf, srf[SRF_SEQ_LIGHT], 0, 0, 0, 0, 33, 30);
		}
		else {
			oapiBlt(surf, srf[SRF_SEQ_LIGHT], 0, 0, 33, 0, 33, 30);
