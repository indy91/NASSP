...
#define XP_LIT_OFF  0.25
	...
	case AID_VC_PANEL14_COMPLIGHTS:
		if (LampToneTestRotary.GetState() == 6) {
			SetCompLight(VC_MAT_L17_CompLight6_DCBus, lca.Fixed_55V_Output.Voltage() / 6.0); // Light On
		}
		else {
	...
	case AID_VC_SEQ_LIGHTS:
		if (scera1.GetVoltage(12, 11) > 2.5 && stage < 2 || LampToneTestRotary.GetState() == 6) {
			SetCompLight(VC_MAT_L19_StageSeq_SysA, lca.Fixed_55V_Output.Voltage() / 6.0); // Light On
		}
		else {
		...
		if (scera1.GetVoltage(12, 12) > 2.5 || LampToneTestRotary.GetState() == 6) {
			SetCompLight(VC_MAT_L20_StageSeq_SysB, lca.Fixed_55V_Output.Voltage() / 6.0); // Light On
		}
		else {
